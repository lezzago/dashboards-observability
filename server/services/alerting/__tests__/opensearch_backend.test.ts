...
describe('HttpOpenSearchBackend', () => {
  ...
  describe('searchAlertHistoryAggregation', () => {
    it('parses time_buckets + by_severity into the expected shape', async () => {
      const { client } = makeClient([
        {
          body: {
            aggregations: {
      ...
      const result = await backend.searchAlertHistoryAggregation(client, { size: 0 });
      expect(result).toEqual({
        indexMissing: false,
        buckets: [
          {
    ...
    it('returns the indexMissing sentinel on index_not_found_exception via body.error.type', async () => {
      ...
      const { client } = makeClient([err]);
      const result = await backend.searchAlertHistoryAggregation(client, { size: 0 });
      expect(result).toEqual({ indexMissing: true });
    });

    it('returns the indexMissing sentinel on a 404', async () => {
    ...
    it('rethrows for non-index-missing errors', async () => {
      const err = new Error('upstream 500') as Error & { statusCode?: number };
      err.statusCode = 500;
      const { client } = makeClient([err]);
      await expect(backend.searchAlertHistoryAggregation(client, { size: 0 })).rejects.toThrow(
        /upstream 500/
      );
    });
  });
