...
import { classifyToHandlerResult, configureErrorExposure, contextFromError } from '../classified_error';
...
describe('classifyToHandlerResult', () => {
  ...
  it('strips sensitive detail by default but keeps it when exposure is enabled', () => {
    const err = new SloRulerError('RULER_VALIDATION_FAILED', 400, 'invalid PromQL at https://host.internal/x');

    const stripped = classifyToHandlerResult(err, { operation: 'rule.create.metric', logger });
    ...
    const exposed = classifyToHandlerResult(err, { operation: 'rule.create.metric', logger });
