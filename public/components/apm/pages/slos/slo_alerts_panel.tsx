...
function buildRulesPath(sloId: string, extraParams?: Record<string, string>): string {
  const params = new URLSearchParams({ slo_id: sloId });
  if (extraParams) {
...
function navigateToAlertManager(sloId: string, extraParams?: Record<string, string>): void {
  const path = buildRulesPath(sloId, extraParams);
