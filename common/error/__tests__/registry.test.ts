...
describe('registry', () => {
  ...
  it('falls back to UNKNOWN when no classifier matches', () => {
    const result = classifyError({ operation: 'op', message: 'boom at https://host.internal/x' });
  ...
  describe('toClientPayload exposure policy', () => {
    const withDetails: ClassifiedError = {
      ...
      retryable: false,
      details: [
        { key: DetailKey.REDACTED, label: 'Details', value: 'boom', sensitivity: 'safe' },
        { key: DetailKey.RAW, label: 'Raw', value: 'boom at https://host.internal/x', sensitivity: 'sensitive' },
      ],
    };
    ...
    it('re-redacts safe detail values as defense in depth', () => {
      const out = toClientPayload({
        ...withDetails,
        details: [
          { key: 'x', label: 'l', value: 'see https://host.cloud/y', sensitivity: 'safe' },
        ],
      });
    ...
    it('adds detail but cannot bypass redaction of safe values', () => {
      ...
      registerErrorDetailEnricher({
        ...
        enrich: (err) => ({
          ...
          details: [
            ...(err.details ?? []),
            { key: 'hint', label: 'Hint', value: 'call https://host.internal/api', sensitivity: 'safe' },
          ],
        }),
      });
