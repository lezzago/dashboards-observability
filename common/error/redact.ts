...
const RULES: RedactionRule[] = [
  ...
  { pattern: /\b\d{1,3}(?:\.\d{1,3}){3}(?::\d+)?\b/g, replacement: '<redacted-ip>' },
  // IPv6 (rough — 3+ hextet groups).
  { pattern: /\b(?:[0-9a-f]{1,4}:){3,}[0-9a-f]{1,4}\b/gi, replacement: '<redacted-ip>' },
  // FQDN-like hosts ending in a generic infra/TLD suffix, optional :port.
  {
    pattern: /\b(?:[a-z0-9-]+\.)+(?:com|net|org|io|dev|internal|local|svc|cluster|cloud|corp)\b(?::\d+)?/gi,
...
// left intact.
const TOKEN_PATTERN = /\b(?=[A-Za-z0-9+/_-]*[A-Za-z])(?=[A-Za-z0-9+/_-]*\d)[A-Za-z0-9+/_-]{24,}={0,2}\b/g;
