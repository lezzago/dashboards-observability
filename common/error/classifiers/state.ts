...
const STATE_CODE_MAP: Record<string, { category: ClassifierResult['category']; code: string; retryable: boolean }> = {
  STATE_RULES_MISSING: { category: 'PARTIAL_STATE', code: ErrorCode.RULES_MISSING, retryable: false },
  STATE_RULES_PARTIAL: { category: 'PARTIAL_STATE', code: ErrorCode.RULES_PARTIAL, retryable: false },
...
export const stateClassifier: ErrorClassifier = {
  ...
  classify: (ctx): ClassifierResult => {
    ...
    return { category: mapped.category, code: mapped.code, retryable: mapped.retryable };
  },
};
...
/**
 * Classify a routing/alertmanager cluster status string. 'ready' is healthy
 * (returns null); anything else — including 'unknown' or '' — surfaces as a
 * partial/unknown routing state instead of the literal word.
 */
export function classifyRoutingStatus(
  ...
  return classifyError({ operation: 'routing.config', ...ctx, upstreamCode: 'STATE_ROUTING_UNKNOWN' });
}
